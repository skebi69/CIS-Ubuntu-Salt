...
{% set ntp = salt['cmd.run']('dpkg-query -l ntp') %}
{% set chrony = salt['cmd.run']('dpkg-query -l chrony') %}

{% if ntp == 'dpkg-query: no packages found matchin ntp'  %}
{% if chrony == 'dpkg-query: no packages found matching chrony'  %}
{{ rule }} install Chrony:
    pkg.installed:  
        - name: chrony
{% else %}
